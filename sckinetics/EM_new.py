...
class ExpectationMaximization_new():
...
def get_constraints(adata, celltype_priors, knn, celltypes):
    ...
    for cellrand in range(num_cells):
        ...
        slopedist = data.iloc[np.where(kNN_graph.toarray()[cellrand,:])[0]].values - data.iloc[cellrand].values
        offset = np.std(slopedist,axis=0) / 10.0
        # clip offset so there are no zero offset values
        offset=np.clip(offset, a_min=np.sort(list(set(offset)))[1], a_max=None)
